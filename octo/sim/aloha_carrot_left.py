...
SOURCE_FRAME_INDICES = (0, 30, 60, 90, 120, 148)
SOURCE_QPOS = (
    (
        0.08590293,
        -0.79613602,
...
SOURCE_EE_POS = (
    (-0.14831511, 0.52902990, 0.22897997),
    (-0.06993151, 0.13056615, 0.01938065),
    (-0.03608171, 0.11919892, 0.00269354),
    (-0.07668623, 0.30182515, 0.09138302),
    (-0.15881469, 0.48945831, 0.24765944),
    (-0.24851412, 0.51542452, 0.29439555),
)
...
class AlohaCarrotLeftConfig:
    ...
    schema: str = "octo.aloha_carrot_left.dataset_aligned.v2"
    seed: int = 42
    ...
    source_reference_dir: str = "outputs/validation/aloha_carrot_sim_rebuild/source_ep0"
    ...
    initial_ee_pos: Tuple[float, float, float] = (
        -0.14831511,
        0.5290299,
        0.22897997,
    )
    source_frame_indices: Tuple[int, ...] = SOURCE_FRAME_INDICES
    source_qpos: Tuple[Tuple[float, ...], ...] = SOURCE_QPOS
    source_ee_pos: Tuple[Tuple[float, float, float], ...] = SOURCE_EE_POS
    mat_center: Tuple[float, float, float] = (0.01, 0.168, 0.021)
    mat_size: Tuple[float, float] = (0.615, 0.35)
    cup_pos: Tuple[float, float, float] = (-0.123, 0.168, 0.062)
    plate_pos: Tuple[float, float, float] = (0.095, 0.168, 0.038)
    carrot_pos: Tuple[float, float, float] = (0.118, 0.145, 0.067)
    ...
    def from_mapping(cls, data: Mapping[str, Any]) -> "AlohaCarrotLeftConfig":
        ...
        return cls(
            ...
            primary_camera_pos=_tuple3(primary.get("position", cls.primary_camera_pos)),
            ...
            wrist_camera_fovy=float(wrist.get("fov_y_degrees", cls.wrist_camera_fovy)),
            ...
            source_frame_indices=tuple(
                int(v)
                for v in left_arm.get(
                    "source_frame_indices",
                    cls.source_frame_indices,
                )
            ),
            source_qpos=tuple(
                tuple(float(v) for v in row)
                for row in left_arm.get("source_qpos", cls.source_qpos)
            ),
            source_ee_pos=tuple(
                _tuple3(row) for row in left_arm.get("source_ee_pos", cls.source_ee_pos)
            ),
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "left_arm": {
                ...
                "source_qpos": [list(row) for row in self.source_qpos],
                "source_ee_pos": [list(row) for row in self.source_ee_pos],
...
class DatasetAlignedController:
    ...
    def __init__(self, config: Optional[AlohaCarrotLeftConfig] = None):
        ...
        self.stage = 0
        self._finish_dwell_remaining = 3
    ...
    def _waypoints(self) -> List[Tuple[np.ndarray, float, float]]:
        ...
        if len(self.config.source_ee_pos) == 6:
            retreat_after_place = _array(self.config.source_ee_pos[4])
            retreat = _array(self.config.source_ee_pos[5])
        else:
            retreat_after_place = _array(self.config.initial_ee_pos)
            retreat = retreat_after_place
        ...
        return [
            ...
            (retreat_after_place, FOLLOWER_GRIPPER_OPEN, 0.018),
            (retreat, FOLLOWER_GRIPPER_OPEN, 0.018),
        ]
    ...
    def is_finished(self, state: SceneState) -> bool:
        ...
        finished = bool(state.success and target_reached and gripper_reached)
        if finished and self._finish_dwell_remaining > 0:
            self._finish_dwell_remaining -= 1
            return False
        return finished
...
class AlohaCarrotLeftSim:
    ...
    def _advance(
        ...
        if (
            object_phase == "on_plate"
            and close_fraction > 0.78
            and distance_to_carrot < 0.055
        ):
            object_phase = "held"
    ...
    def _draw_wrist_carrot_close(
        ...
        *,
        ...
        if over_cup:
            ...
            self._draw_wrist_vertical_carrot(
                draw, center_x=332, top=168, bottom=405, scale=1.18
            )
        else:
    ...
    def _draw_wrist_place_close(self, draw: ImageDraw.ImageDraw) -> None:
        ...
        draw.rounded_rectangle(
            (593, 426, 617, 510), radius=11, fill=(239, 126, 40, 215)
        )
        self._draw_wrist_fingers(draw, mode="wide")
    ...
    def _draw_wrist_mat_texture(
        ...
        *,
        ...
        for x in range(x1 - 20, x2 + 30, 18):
            draw.line(
                (x, y1, x + 35, y2), fill=(200, 45, 108, max(20, alpha // 2)), width=1
            )
    ...
    def _draw_wrist_plate(
        ...
        *,
        ...
        draw.ellipse(
            (x - rx - 5, y - ry + 10, x + rx + 8, y + ry + 20), fill=(0, 0, 0, 34)
        )
        ...
        draw.arc(
            (x - rx + 13, y - ry + 7, x + rx - 13, y + ry - 7),
            190,
            330,
            fill=(230, 255, 235, 120),
            width=3,
        )
    ...
    def _draw_wrist_vertical_carrot(
        ...
        *,
        ...
        draw.ellipse(
            (center_x - width, bottom - 9, center_x + width, bottom + 16),
            fill=(0, 0, 0, 34),
        )
        draw.rounded_rectangle(
        ...
        seam_y = bottom - cap - 8
        draw.line(
            (center_x - width + 2, seam_y, center_x + width - 2, seam_y),
            fill=(154, 71, 29, 145),
            width=2,
        )
        for y in range(top + 42, bottom - cap - 14, max(24, int(30 * scale))):
            draw.line(
                (center_x - width + 8, y, center_x + width - 11, y - 9),
                fill=(255, 197, 97, 130),
                width=2,
            )
    ...
    def _draw_wrist_fingers(self, draw: ImageDraw.ImageDraw, *, mode: str) -> None:
        ...
        draw.line(
            (left_line[0] + 15, left_line[1] - 34, left_line[2] - 8, left_line[3] + 12),
            fill=(86, 90, 87, 90),
            width=6,
        )
        draw.line(
    ...
    def _reward(
        self, object_phase: str, ee_pos: np.ndarray, carrot_pos: np.ndarray
    ) -> float:
        if object_phase == "in_cup":
            return 4.0
        if object_phase == "held":
    ...
    def _qpos_from_ee(self, ee_pos: np.ndarray, gripper: float) -> np.ndarray:
        ...
        home = np.asarray(self.config.left_home_qpos[:6], dtype=np.float64)
        qpos[:6] = [
            _clamp(home[0] + dx * 1.55 - dy * 0.25, -1.45, 1.45),
            _clamp(home[1] + dz * 3.1 + dy * 0.55, -1.55, 0.65),
            _clamp(home[2] - dz * 2.2 + dx * 0.38, -0.4, 1.55),
            _clamp(home[3] + dx * 0.75, -1.8, 1.8),
            _clamp(home[4] - dz * 1.4, -1.3, 1.1),
            _clamp(home[5] + dx * 0.25, -1.6, 1.6),
        ]
        qpos[6:] = gripper
        return qpos
    ...
    def _draw_primary_arm(self, draw: ImageDraw.ImageDraw, state: SceneState) -> None:
        ...
        draw.line(
            (elbow[0], elbow[1], wrist[0], wrist[1]), fill=(10, 10, 10, 255), width=18
        )
        open_fraction = gripper_open_fraction(state.gripper)
        spread = 10 + 24 * open_fraction
        tip = (ee_px[0], ee_px[1])
        draw.line(
            (wrist[0], wrist[1], tip[0] - spread, tip[1] + 16),
            fill=(0, 0, 0, 255),
            width=8,
        )
        draw.line(
    ...
    def _draw_cup(self, draw: ImageDraw.ImageDraw, state: SceneState) -> None:
        ...
        draw.rounded_rectangle(
            ...
            radius=12,
            fill=(82, 206, 231, 255),
            outline=(33, 155, 184, 220),
            width=2,
        )
        draw.ellipse(
            (x - 34, y - 30, x + 33, y + 4),
            fill=(100, 224, 242, 255),
            outline=(31, 150, 176, 235),
            width=3,
        )
    ...
    def _draw_carrot(
        ...
        *,
        ...
        draw.ellipse(
            (x - 32, y - 12, x - 7, y + 13),
            fill=(255, 153, 62, 255),
            outline=(179, 84, 24, 210),
            width=1,
        )
        for offset in (-12, 2, 15):
            draw.line(
                (x + offset, y - 10, x + offset + 4, y + 11),
                fill=(255, 196, 95, 150),
                width=3,
            )
        draw.polygon(
...
def rollout_metrics(states: Sequence[SceneState]) -> Dict[str, Any]:
    ...
    return {
        ...
        "pre_grasp_carrot_translation_std": float(
            pre_grasp_positions.std(axis=0).max()
        ),
...
def source_aligned_timeline(
    ...
    on_plate = [
        idx for idx, state in enumerate(states) if state.object_phase == "on_plate"
    ]
    ...
    transfer_idx = held[min(len(held) - 1, max(0, int(round(len(held) * 0.65))))]
    if len(config.source_ee_pos) == 6:
        source_place_ee = _array(config.source_ee_pos[4])
        place_idx = min(
            in_cup[:-1] or in_cup,
            key=lambda idx: float(np.linalg.norm(states[idx].ee_pos - source_place_ee)),
        )
    else:
        place_offset = min(
            max(0, len(in_cup) - 2),
            int(round(max(0, len(in_cup) - 1) * 0.85)),
        )
        place_idx = in_cup[place_offset]
...
def save_gif(
    path: Path | str, frames: Iterable[np.ndarray], *, duration_ms: int = 80
) -> None:
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _draw_room(width: int, height: int) -> Image.Image:
    ...
    table[..., 0] = np.clip(
        203 + 14 * np.sin(x / 25) + 9 * np.sin((x + y) / 41), 0, 255
    )
    ...
    draw = ImageDraw.Draw(image, "RGBA")
    draw.polygon(
        [(0, 0), (640, 0), (640, 120), (530, 92), (112, 93), (0, 132)],
        fill=(0, 125, 90, 255),
    )
...
def _draw_wrist_room(width: int, height: int) -> Image.Image:
    ...
    table = np.zeros((height, width, 3), dtype=np.uint8)
    table[..., 0] = np.clip(
        206 + 16 * np.sin(x / 34) + 7 * np.sin((x + y) / 55), 0, 255
    )
